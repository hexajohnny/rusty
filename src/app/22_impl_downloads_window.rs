impl AppState {
    fn draw_transfer_progress_bar(
        ...
        let bar_height = (ui.spacing().interact_size.y - 2.0).max(16.0);
        let (rect, _) =
    ...
    fn draw_downloads_manager_window(&mut self, ctx: &egui::Context) {
        ...
        let mut open = self.downloads_window_open;
                            ...
                            ui.group(|ui| {
                                ui.horizontal(|ui| {
                                    ...
                                    ui.with_layout(egui::Layout::right_to_left(Align::Center), |ui| {
                                        let cancel_icon = egui::Image::new(
                                            egui::include_image!("../../assets/x.png"),
                                        )
                                        .tint(self.theme.fg);
                                        let cancel_hover_text = if matches!(
                                            job.state,
                                        ...
                                        let folder_icon = egui::Image::new(
                                        ...
                                        if matches!(job.state, DownloadState::Failed | DownloadState::Canceled) {
                                            let retry_icon = egui::Image::new(
                                                egui::include_image!("../../assets/retry.png"),
                                            )
                                            .tint(self.theme.fg);
                                            let retry_hover_text = match job.direction {
                                                TransferDirection::Download => {
                                                    "Retry and resume partial download"
                                                }
                                        ...
                                        let trash_icon = egui::Image::new(
                                            egui::include_image!("../../assets/trash.png"),
                                        )
                                        .tint(self.theme.fg);
                                        if title_bar_image_button(
                                            ui,
                                        ...
                                        .clicked()
                                        {
                                            remove_ids.push(job.request_id);
                                        }
                                    });
                                });
                                ...
                                if !job.message.trim().is_empty() {
                                    ui.label(egui::RichText::new(&job.message).color(self.theme.muted));
                                }
                            });
                            ui.add_space(6.0);
                        }
        ...
        // Closing hides the manager; it does not cancel running downloads.
        self.downloads_window_open = open;
    }
}
